...
//按单月分表
@Component
public class DateShardingAlgorithmTwo implements StandardShardingAlgorithm<Long> {
    // 查询使用
    @Override
    public Collection<String> doSharding(Collection<String> collection, RangeShardingValue<Long> rangeShardingValue) {
        collection.forEach(i -> System.out.println("节点配置表名为: " + i));
        // 查询数据库中的表 hss_history
        List<String> tableNames = ShardingAlgorithmToolTwo.getAllTableNameBySchema();
        tableNames.forEach(i -> System.out.println("数据库实时表名: " + i));
        // 查询缓存中的表 hss_history
        HashSet<String> tableNameCache = ShardingAlgorithmToolTwo.cacheTableNames();
        ...
        for (String tableName : tableNameCache) {
            if (!tableName.equals("main_order_two")) {
                // 截取缓存表名后缀的年月 yyyyMM
                String num = tableName.split("_")[3];
                // 在查询条件范围内才添加
    ...
    public String doSharding(Collection<String> collection, PreciseShardingValue<Long> preciseShardingValue) {
        ...
        return ShardingAlgorithmToolTwo.shardingTablesCheckAndCreatAndReturn(logicTableName, resultTableName.toString());
    }
