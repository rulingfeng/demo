...
public class CountBloomFilter<T> extends StandardBloomFilter<T> {
	protected AdvanceBitSet counts;
	protected int c;
	...
	// Using c = 4 is the best choice for most applications
	public CountBloomFilter(double n, double p) {
		this(optimalM(n, p), optimalK(n, optimalM(n, p)), 4);
	}
	 ...
	 */
	public CountBloomFilter(double n, double p, int c) {
	 ...
	 *            Number of bits used for counting
	 */
	public CountBloomFilter(int m, int k, int c) {
		this(new AdvanceBitSet(m * c), new AdvanceBitSet(m), m, k, c);
	}

	protected CountBloomFilter(AdvanceBitSet counts, AdvanceBitSet bloom, int m, int k, int c) {
		super(bloom, m, k);
		this.counts = counts;
		this.c = c;
	}
	
	protected CountBloomFilter() {
	...
	@Override
	public boolean union(StandardBloomFilter<T> other) {
		// FIXME
		return false;
	}

	@Override
	public boolean intersect(StandardBloomFilter<T> other) {
		// FIXME
		return false;
	}
	...
	public CountBloomFilter<T> clone() {
		CountBloomFilter<T> o = (CountBloomFilter<T>) super.clone();
		if(this.counts != null)
			o.counts = (AdvanceBitSet) this.counts.clone();
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		CountBloomFilter other = (CountBloomFilter) obj;
		if (c != other.c)
			return false;
		if (counts == null) {
