 ...
 * <i>getbit</i> operations of Redis.
 * The performance of this data structure is very good, as operations are grouped
 * into fast transactions, minimizing the network overhead of all bloom filter operations to one round trip to Redis.
 * 
 * @param <T>
 */
public class RedisBloomFilter<T> extends StandardBloomFilter<T> {
    private String name;
    ...
    public static <V> RedisBloomFilter<V> loadFilter(Jedis jedis, String name) {
        if (!jedis.exists(name)) return null;
        //TODO:
        Pipeline p = jedis.pipelined();
        p.multi();
        p.hget(buildConfigKeyName(name), M_KEY);       //0
        p.hget(buildConfigKeyName(name), K_KEY);       //1
        p.hget(buildConfigKeyName(name), TYPE_KEY);    //2
        p.hget(buildConfigKeyName(name), HASH_KEY);    //3
        p.hget(buildConfigKeyName(name), N_KEY);    //4
        ...
        if (separatorOffset >= 0) {
            ...
            return new RedisBloomFilter<V>(name, jedis, m, k, cryptoMethod, populationType, n);
        } else {
            return new RedisBloomFilter<V>(name, jedis, m, k, HashMethod.valueOf(hashType), populationType, n);
        }
    ...
    public static boolean removeFilter(Jedis jedis, String name) {
        ...
        p.multi();
        p.del(name);
        p.del(RedisBloomFilter.buildConfigKeyName(name));
        p.del(RedisBloomFilter.buildPopulationKeyName(name));
        p.exec();
        p.sync();
    ...
    public static <V> RedisBloomFilter<V> createFilter(Jedis jedis, String name, double n, double p) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = (int)n;
        return new RedisBloomFilter<V>(name, jedis, optimalM(n, p), optimalK(n, optimalM(n, p)), StandardBloomFilter.HashMethod.Murmur, false, size);
    }
    
    public static <V> RedisBloomFilter<V> createFilter(Jedis jedis, String name, double n, double p, HashMethod method) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = (int)n;
        return new RedisBloomFilter<V>(name, jedis, optimalM(n, p), optimalK(n, optimalM(n, p)), method, false, size);
    }

    public static <V> RedisBloomFilter<V> createPopulationFilter(Jedis jedis, String name, double n, double p) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = (int)n;
        return new RedisBloomFilter<V>(name, jedis, optimalM(n, p), optimalK(n, optimalM(n, p)), StandardBloomFilter.HashMethod.Murmur, true, size);
    }
    
    public static <V> RedisBloomFilter<V> createPopulationFilter(Jedis jedis, String name, double n, double p, HashMethod method) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = (int)n;
        return new RedisBloomFilter<V>(name, jedis, optimalM(n, p), optimalK(n, optimalM(n, p)), method, true, size);
    }

    public static <V> RedisBloomFilter<V> createFilter(Jedis jedis, String name, int m, int k, HashMethod method) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = 0;       // no way of knowing how many elements we sized it for
        return new RedisBloomFilter<V>(name, jedis, m, k, method, false, size);
    }

    public static <V> RedisBloomFilter<V> createPopulationFilter(Jedis jedis, String name, int m, int k, HashMethod method) {
        if (jedis.exists(name)) throw new IllegalArgumentException("Filter with this name already exists.");
        int size = 0;       // no way of knowing how many elements we sized it for
        return new RedisBloomFilter<V>(name, jedis, m, k, method, true, size);
    }
     ...
     */
    protected RedisBloomFilter(String name, Jedis jedis, int m, int k, HashMethod method, boolean hasPopulation, int size) {
        ...
        this.useConnection(jedis);
    }

    protected RedisBloomFilter(String name, Jedis jedis, int m, int k, String method, boolean hasPopulation, int size) {
        super(new RedisBitSet(jedis, name, m), m, k);
        this.jedis = jedis;
        this.name = name;
        this.hasPopulationCount = hasPopulation;
