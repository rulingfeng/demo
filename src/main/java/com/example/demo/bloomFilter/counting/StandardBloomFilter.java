...
public class StandardBloomFilter<T> implements Cloneable, Serializable {
	protected AdvanceBitSet bloom;
	...
	protected HashMethod hashMethod = HashMethod.Murmur; // Default Hash Method
	 ...
	 *            Tolerable false positive rate
	 */
	public StandardBloomFilter(double n, double p) {
	 ...
	 */
	public StandardBloomFilter(int m, int k) {
		this(new AdvanceBitSet(m), m, k);
	}

	protected StandardBloomFilter(AdvanceBitSet bloom, int m, int k) {
		this.m = m;
		this.bloom = bloom;
		this.k = k;
	 ...
	 */
	public StandardBloomFilter(AdvanceBitSet bloomFilter, int m, int k, HashMethod hashMethod,
    ...
    public List<Boolean> contains(Collection<T> values) {
        List<Boolean> does = new ArrayList<Boolean>();
        for (T value: values) {
	...
	public AdvanceBitSet getBitSet() {
		return bloom;
	}
	...
	protected int[] hashCrypt(byte[] value) {
		...
		while (computedHashes < k) {
			...
			AdvanceBitSet hashed = AdvanceBitSet.valueOf(digest);
			...
			for (int split = 0; split < (hashBits / filterSize)
				...
				AdvanceBitSet hashSlice = hashed.get(from, to);
				// Bitset to Int
	 ...
	 */
	public synchronized boolean union(StandardBloomFilter<T> other) {
	 ...
	 */
	public synchronized boolean intersect(StandardBloomFilter<T> other) {
		if (compatible(this, other)) {
	...
	protected boolean compatible(StandardBloomFilter<T> bloomFilter,
			StandardBloomFilter<T> other) {
	...
	public synchronized Object clone() {
		StandardBloomFilter<T> o = null;
		try {
			o = (StandardBloomFilter<T>) super.clone();
		} catch (CloneNotSupportedException e) {
		}
		o.bloom = (AdvanceBitSet) bloom.clone();
	...
	public synchronized boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		StandardBloomFilter<T> other = (StandardBloomFilter<T>) obj;
		if (bloom == null) {
			if (other.bloom != null)
				return false;
    ...
    public static void main(String[] args) 
    {
		...
		for( int i=0 ; i < (expectedNoOfElements*precentageOfActualInsertation); i++ )
		{
		...
		for( int i=0 ; i < (expectedNoOfElements); i++ )
		{
			if(bf.contains("falseString"+i)){
				countFalsePositive++;
			}
		}
